# Angular Landing Hostel Tarnopil industry Template (SSR + Prerender)
...
```bash
git clone https://github.com/IT-Kamianets/hostel-tarnopil.git PROJECT_NAME
cd PROJECT_NAME
npm i
npm run start
```
...
git add .
git commit -m "chore(init): bootstrap project from hostel-tarnopil template"
```
...
```text
chore(init): bootstrap project from hostel-tarnopil template
```
